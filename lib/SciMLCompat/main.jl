...
f = create_right_hand_side_inplace(setup, psolver)
du = similar(u0)
f(du,u0, nothing, 0.0)
...
f = create_right_hand_side_enzyme(get_backend(u0), setup, T, n)
P = ComponentArray(f=zeros(T, (N,N,2)),div=zeros(T,(N,N)), p=zeros(T,(N,N)), ft=zeros(T,n*n+1), pt=zeros(T,n*n+1))
f(du,u0, nothing, 0.0)
...
using Plots
p1 = bar(["SciML", "SciML inplace", "Enzyme"], [usedtime, usedtime_ip, usedtime_e], title = "Time comparison", ylabel = "Time (s)", legend = false)
p2 = bar(["SciML", "SciML inplace", "Enzyme"], [allocation, allocation_ip, allocation_e], title = "Allocation comparison", ylabel = "Memory (MB)", legend = false)
p3 = bar(["SciML", "SciML inplace", "Enzyme"], [gc, gc_ip, gc_e], title = "GC comparison", ylabel = "GC (s)", legend = false)
