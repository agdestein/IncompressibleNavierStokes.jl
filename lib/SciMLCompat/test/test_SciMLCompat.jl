...
@timed Enzyme.autodiff(Enzyme.Reverse, apriori, Active, DuplicatedNoNeed(u, du), DuplicatedNoNeed(P, dP), DuplicatedNoNeed(temp, dtemp))
...
dP = Enzyme.make_zero(P);
du = Enzyme.make_zero(u);
@timed Enzyme.autodiff(Enzyme.Reverse, loss, DuplicatedNoNeed(l, dl), DuplicatedNoNeed(P, dP), DuplicatedNoNeed(u, du), Const(trange), Const(saveat))
# check the gradient
@assert sum(dP.θ)!=0
