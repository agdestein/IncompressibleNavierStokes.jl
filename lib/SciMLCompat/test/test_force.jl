...
sol, time_op, allocation_op, gc_op, memory_counters_op =
    @timed solve(prob, Tsit5(); dt = dt, saveat = saveat);
...
sol_ip, time_ip, allocation_ip, gc_ip, memory_counters_ip =
    @timed solve(prob_ip, Tsit5(); dt = dt, saveat = saveat);
...
sol_e, time_e, allocation_e, gc_e, memory_counters_e =
    @timed solve(prob_e, Tsit5(); dt = dt, saveat = saveat);

# Test the forces
@test op_t_1 ≈ ip_t_1 ≈ e_t_1
# Test the solutions 
@test sol.u[end] ≈ sol_ip.u[end] ≈ sol_e.u[end]
# Test the time
