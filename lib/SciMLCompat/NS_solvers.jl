...
(state, outputs), time_ins2, allocation2, gc2, memory_counters2 =
    @timed INS.solve_unsteady(;
        setup,
        ustart,
        tlims = trange,
        Δt = dt,
        processors = (
            ...
            anim = INS.animator(;
                setup,
                path = "./lib/SciMLCompat/plots/vorticity_INS.mkv",
                nupdate = savevery,
            ),
...
_, time_ins, allocation, gc, memory_counters =
    @timed INS.solve_unsteady(; setup, ustart, tlims = trange, Δt = dt);
...
p1 = Plots.bar(
    ...
    legend = false,
    series_annotations = [time_ins, time_ode],
);
#Memory allocation
p2 = Plots.bar(
    ["INS", "SciML"],
