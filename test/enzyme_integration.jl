...
@testmodule Info begin
    ...
    boundary_conditions = (; u = ntuple(d -> (DirichletBC(), DirichletBC()), D))
...
@testitem "Enzyme one force pullback" setup = [Info, EnzymeSnipPull] begin
    (; setup, psolver, T, N, viscosity) = Info
    dudt = zeros(T, (N, N, 2))
    u = rand(T, (N, N, 2))
    u0 = copy(u)
    ...
    @test all(!isnan, F_out(u, (; viscosity), T(0)))
    ...
    _, tz, mz = @timed begin
        for i = 1:niter
            dudt = F_out(list_u[i], (; viscosity), T(0))
            push!(list_z, dudt)
        end
    end
    list_e = [zeros(T, (N, N, 2)) for i = 1:niter]
    _, te, me = @timed begin
        for i = 1:niter
