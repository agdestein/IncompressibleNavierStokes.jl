...
function operator_interpolation!(setup)
    ...
    Iv_uy_bc_lu = (;
        Iv_uy_bc_lu...,
        B3D = I(Nz) ⊗ Iv_uy_bc_lu.B1D ⊗ I(Nvx_in),
    )
    Iv_uy_bc_lu = (;
        Iv_uy_bc_lu...,
        Bbc = I(Nz) ⊗ Iv_uy_bc_lu.Btemp ⊗ I(Nvx_in),
    )
    ...
    Iv_uy_bc_lr = (;
        Iv_uy_bc_lr...,
        B3D = I3D * (I(Nz) ⊗ I(Nuy_t - 1) ⊗ Iv_uy_bc_lr.B1D),
    )
    Iv_uy_bc_lr = (;
        Iv_uy_bc_lr...,
        Bbc = I3D * (I(Nz) ⊗ I(Nuy_t - 1) ⊗ Iv_uy_bc_lr.Btemp),
    )
    ...
    Iw_uz_bc_lr = (;
        Iw_uz_bc_lr...,
        B3D = I3D * (I(Nz + 1) ⊗ I(Ny) ⊗ Iw_uz_bc_lr.B1D),
    )
    Iw_uz_bc_lr = (;
        Iw_uz_bc_lr...,
        Bbc = I3D * (I(Nz + 1) ⊗ I(Ny) ⊗ Iw_uz_bc_lr.Btemp),
    )
    ...
    ## Iv_wy
    diag1 = fill(weight, Nwz_t)
    I1D = spdiagm(Nwz_t - 1, Nwz_t, 0 => diag1, 1 => diag1)
    I1D = Bwvz * I1D * mat_hz2
    ...
    # Resulting operator:
    Iw_vz = Iv_wy_bc_bf.B3D * Iv_wy_bc_lu.B3D
